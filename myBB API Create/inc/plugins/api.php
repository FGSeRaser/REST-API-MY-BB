...
/**
 * Author: Nick Bruiser
 * Copyright 2026 NickBruiser, All Rights Reserved
 *
 * Website: 
...
function api_info()
{
    return [
        'name' => 'MyBB API',
        'description' => 'Easily add an API to your Board to Manage your Board',
        'website' => '',
        'author' => 'Nick Bruiser',
        'authorsite' => '',
        'version' => '1.2',
...
if (!$db->field_exists('api_key', 'users')) {
    $db->add_column('users', 'api_key', "varchar(32) DEFAULT NULL");
}

if (!$db->field_exists('api_key_updated', 'users')) {
    $db->add_column('users', 'api_key_updated', "int(11) DEFAULT NULL");
}
...
{
    global $db, $mybb, $templates, $headerinclude, $usercpnav, $header, $footer, $api_key, $error;
    ...
    if ($mybb->input['action'] == 'api_key') {
        ...
        if ($mybb->request_method == 'post') {
            ...
            if (!can_generate_key()) {
                $error =
                    'Du hast nicht die Berechtigung um dir einen Schlüssel zu erstellen.';
            }
        ...
        eval(
    "\$api_usercp_manage = \"" . $templates->get('api_usercp_manage') . "\";"
);
